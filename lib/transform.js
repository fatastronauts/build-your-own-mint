...
exports.transformBalancesToUpdates = balances => {
  const now = moment();

  return balances.reduce((acc, balance, idx) => {
    return acc.concat([
      { range: `${SHEET_NAMES.BALANCES}!A${idx + 2}`, value: balance.name },
      { range: `${SHEET_NAMES.BALANCES}!B${idx + 2}`, value: balance.balance },
    ]);
  }, []);
};
