...
const { ObjectWithAccessCounter } = require('./helpers');
...
const accountMapping = new ObjectWithAccessCounter(require('../accounts.secret')); // THIS FILE SHOULD BE GITIGNORED
...
exports.transformBalancesToUpdates = accounts => {
  ...
  accountMapping.getUncalledKeys().forEach(([, column]) => {
    const idx = ALPHABET.indexOf(column);
