...
exports.fetchTransactions = async function() {
  const rawTransactions = await Promise.all(
        ...
        .then(({ accounts, transactions }) => ({
          transactions: transactions.map(el => {
            const matchedAccount = accounts.find(
              account => account.account_id === el.account_id,
            );
            return {
              ...el,
              account:
    ...
    throw new Error("Fetching transactions didn't work");
  });

  // concat all transactions
  return rawTransactions.reduce((all, { transactions }) => {
    return all.concat(
      transactions.map(({ name, date, amount, category, account }) => ({
        name,
        date,
        amount: -amount,
        category,
...
exports.fetchBalances = async function() {
  const rawBalances = await Promise.all(
    ...
    throw new Error("Fetching balances didn't work");
  });

  return rawBalances.reduce((all, { accounts }) => {
    return all.concat(
      accounts.map(({ balances, official_name, name, type }) => ({
